...
#                 else:
#                     if weight_decay != 0: p.mul_(1 - adam_lr * weight_decay)
...
# # ============================================================================ #
# @torch.no_grad()
# def greedy_omp(mu, K, kappa, lr_inner=0.1, steps=20):
#     """
#     Algorithm 2: GREEDYOMP
...
#         # g = mu - eta * K @ weights (eta absorbed into K formulation in paper)
...
#         # APGA inner solve (simplified to Projected Gradient Ascent on support)
#         for _ in range(steps):
#             g = mu - torch.matmul(K, weights)
#             # Only update weights in support
#             for idx in support:
#                 weights[idx] += lr_inner * g[idx]
...
# # ============================================================================ #
# #                      Lightning Module (MANUAL OPTIMIZATION)                  #
# # ============================================================================ #
# class LlamaLoRAMuGreatsModule(pl.LightningModule):
#     def __init__(self, model_name, lora_config, lr, adam_w_lr, weight_decay, kappa_ratio=0.125):
#         super().__init__()
#         self.save_hyperparameters(ignore=["lora_config"])
#         self.lr = lr
#         self.adam_w_lr = adam_w_lr
#         self.weight_decay = weight_decay
#         self.kappa_ratio = kappa_ratio # e.g., select 16 out of 128 (12.5%)
        
#         # Disable automatic optimization to manually perform GREATS selection
...
#         # Projection settings (p = hidden_size usually, per paper)
#         self.fjlt = None 

#     def _get_target_lora_param(self):
#         """Helper to get a single LoRA weight matrix for fast selection scoring (per paper section 7.1)"""
#         # Finds the last layer's value projection LoRA A matrix
...
#         # Calculate kappa (number of items to select)
#         kappa = max(1, int(batch_size * self.kappa_ratio))
        
#         target_param = self._get_target_lora_param()
#         original_dim = target_param.numel()
        
#         # Initialize FJLT compressor on first step
#         if self.fjlt is None:
#             p_dim = self.model.config.hidden_size # Project to embed dim
#             self.fjlt = FastJohnsonLindenstrauss(original_dim, p_dim, target_param.device, target_param.dtype)

#         # ------------------------------------------------------------------ #
#         # STEP 1 & 2: Get Validation Gradients (Simulated here with subset)  #
#         # In a real distributed setting, you fetch this from a Val Dataloader#
#         # ------------------------------------------------------------------ #
#         # For demonstration, we split the batch: 1 sample for val, rest for train
#         if batch_size < 4:
#             # Fallback if batch is too small: just train on it
...
#         val_idx = 0
#         val_input = input_ids[val_idx:val_idx+1]
#         val_mask = attention_mask[val_idx:val_idx+1]
...
#         val_loss = self(val_input, val_mask, labels=val_input).loss
#         val_grad = torch.autograd.grad(val_loss, target_param, retain_graph=True)[0]
        
#         # Project Validation to Stiefel Tangent & Compress
#         val_phi = project_to_stiefel_tangent(target_param, val_grad)
#         val_phi_compressed = self.fjlt.compress(val_phi.flatten())
        
#         # ------------------------------------------------------------------ #
#         # STEP 3: Get Per-Example Gradients for the Candidates               #
#         # ------------------------------------------------------------------ #
#         # NOTE: Using a loop for pedagogical clarity. In production, use 
#         # torch.func.vmap or opacus to get per-example gradients efficiently.
#         candidate_phis = []
#         for i in range(candidate_size):
#             ex_input = train_input[i:i+1]
#             ex_mask = train_mask[i:i+1]
#             ex_loss = self(ex_input, ex_mask, labels=ex_input).loss
#             ex_grad = torch.autograd.grad(ex_loss, target_param, retain_graph=True)[0]
            
#             ex_phi = project_to_stiefel_tangent(target_param, ex_grad)
#             ex_phi_compressed = self.fjlt.compress(ex_phi.flatten())
#             candidate_phis.append(ex_phi_compressed)
            
#         candidate_phis = torch.stack(candidate_phis) # [B_candidates, p]
        
#         # ------------------------------------------------------------------ #
#         # STEP 4: Build mu and K (Gram Matrix)                               #
#         # ------------------------------------------------------------------ #
#         # K = Phi @ Phi^T
#         K = torch.matmul(candidate_phis, candidate_phis.t()) # [B, B]
        
#         # mu = Phi @ val_phi + (eta/2) * diag(K)
#         eta = self.lr 
#         phi_dot_val = torch.matmul(candidate_phis, val_phi_compressed) # [B]
#         mu = phi_dot_val + (eta / 2.0) * torch.diag(K)
        
#         # ------------------------------------------------------------------ #
#         # STEP 5: GREEDYOMP Selection                                        #
#         # ------------------------------------------------------------------ #
#         selected_relative_indices = greedy_omp(mu, K, kappa)
        
#         # Map back to original batch indices (shift by 1 because index 0 was val)
#         selected_indices = [idx + 1 for idx in selected_relative_indices]
        
#         # ------------------------------------------------------------------ #
#         # STEP 6: Actual Forward/Backward on Selected Subset                 #
#         # ------------------------------------------------------------------ #
#         opt.zero_grad() # Clear the gradients accumulated during selection
...
#         self.log("subset_selected", float(kappa), prog_bar=True)
...
#     # Needs a larger micro_batch_size so GREATS has candidate choices
#     parser.add_argument("--micro_batch_size", type=int, default=32) 
#     parser.add_argument("--kappa_ratio", type=float, default=0.25) # e.g., select 8 out of 32
...
#     print(f"\n{'='*70}")
#     print(f"[START] Llama3-8B LoRA + Muon + mu-GREATS Online Batch Selection")
#     print(f"{'='*70}\n")
...
#         mu = torch.matmul(candidate_phis, val_phi_compressed) + (self.hparams.lr / 2.0) * torch.diag(K)
...
def main():
    ...
    # W&B Logger setup
    wandb_logger = WandbLogger(project="exp-1a", name="Exp-1a_muon")
